...
Creating the closure: Each state needs to be "closed" meaning that every item contining ". X" needs to add all rules in X to the closure. We need to consider potential empty rules

## Running the parser

At the surface level, the compiler has produced a function that simply takes a block of text (via pointers/iterators to the text) and produces a parse tree. The tree is stored in a `tree` object, that has been optimized for efficiency.
